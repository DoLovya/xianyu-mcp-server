...
- **方式一：直接写入完整 Cookie**：在 `env` 中填写 `XIANYU_COOKIE`
- **方式二：Cookie 存放在单独文件中**：在 `env` 中填写 `XIANYU_COOKIE_FILE` 指向文件路径（支持相对路径和绝对路径）

优先级：配置了 `XIANYU_COOKIE` 时，`XIANYU_COOKIE_FILE` 会被忽略。
...
- 先保持 `env` 为空启动 MCP
- 启动后服务会自动进入"首次配置模式"，在本机打开一个网页展示二维码（`127.0.0.1`），扫码成功后会自动把 Cookie 写回
...
- 可选：调用 `qr_login_save_env` 将 Cookie 写入当前工作目录 `.env`（避免手动复制；写入后通常无需重启，下一次工具调用会自动读取新值）

首次配置模式相关开关（可选，通过 `env` 设置）：
...
推荐优先使用 `XIANYU_COOKIE_FILE` 指向一个本地文件路径，避免把 Cookie 写进配置文件。

各客户端差异：

| 客户端         | 配置文件路径                                                               | 支持 `${workspaceFolder}` | 备注                                                                 |
| -------------- | -------------------------------------------------------------------------- | ------------------------- | -------------------------------------------------------------------- |
| Trae           | `.trae/mcp.json`（项目级）或 `~/.trae/mcp.json`（全局）                    | 项目级支持                | 配置后重载工作区；Trae 会基于 `env` 中出现的键渲染输入框              |
| Claude Desktop | `~/Library/Application Support/Claude/claude_desktop_config.json`（macOS） | 否                        | 保存后重启                                                           |
| Cursor         | `.cursor/mcp.json`（项目级）或 `~/.cursor/mcp.json`（全局）                | 项目级支持                | 全局配置需使用完整路径                                               |
| VS Code        | `.vscode/mcp.json`                                                         | 是                        | 使用 `"servers"` 字段（非 `"mcpServers"`），需显式 `"type": "stdio"`；需 VS Code 1.102+ |
| Cherry Studio  | UI 配置，无配置文件                                                        | N/A                       | 设置 → MCP 服务器 → 添加，类型选 STDIO，`command` 填 `uvx`，`args` 填 `--from xianyu-mcp-server xianyu-mcp-server` |

- `xianyu-mcp-server` 只是 MCP 服务名，可以自定义
- `command` 优先使用 `uvx`，可使用绝对路径，例如 `/Users/<user>/.trae/tools/uv/latest/bin/uvx`
- Windows 路径使用反斜杠，例如 `C:\Users\<user>\<path>\cookie.txt`

HTTP 模式（可选）：在 `args` 末尾追加 `"--http"` 启动后，监听 `http://localhost:8000/mcp`，Cherry Studio 等客户端可选 SSE 或 HTTP 类型接入。
...
请先确认 uv 已安装并在 PATH 中可用：

```bash
uv --version
```

若未安装，见[环境要求](#环境要求)。若已安装仍报错，可使用更稳的等价写法：

```json
{
  "command": "uv",
  "args": ["tool", "run", "--from", "xianyu-mcp-server", "xianyu-mcp-server"]
}
```
...
- `command` 或 `args` 写错（`uvx` 必须有 `--from xianyu-mcp-server xianyu-mcp-server`）
- uv / uvx 不在客户端运行时的 PATH 中，改用绝对路径或使用 `"command": "uv"` + `"tool", "run"` 写法
...
当前实现会在每次工具调用前重新读取环境变量或 `.env`。通常只要改的是：

- MCP 配置 `env` 中的 `XIANYU_COOKIE`
